...
class Pagination {
  constructor(props) {
    ...
    this.dataPerPage = dataPerPage || 15;
  ...
  render() {
    ...
    for (let i = firstPage; i <=lastPage; i++) {
      pagingHtml += `
        <li class="pagination_num ${i === this.currentPage ? 'active' : ''}" data-id="${i}"><a href="/}">${i}</a></li>
      `;
    }
  ...
  return pagingHtml;
    ...
    usePagination(){
      ...
      document.getElementById('prev').addEventListener('click', () => {
        const prevGroupLastPage = Math.max((pageGroup - 1) * this.pagingPerPage, 1);
  ...
  //페이지네이션 내 버튼을 클릭했을 때 버튼 액션에 해당하는 페이지 값을 얻어와서 페이지네이션 렌더링
    gotoPage(page) {
      ...
      document.querySelector('.pagination_container').innerHTML = this.render(page);
      this.usePagination();
    }
  }
